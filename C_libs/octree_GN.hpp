...
//============================================================================
// octree_GN.h -*- C++ -*-; Octree for objects with 'coord' attribute
//
...
//! Der Typ T MUSS ein Attribut 'vec3d<float> coord' haben !!!
template<class T>
class OCTREE {
    public:
        vector<T*> objects;
        ...
        OCTREE(vector<stl_ptr<T> > const& objs,float const& r,float const& extra_range=-1.,OCTREE<T>* pp=0) :
            ...
            for (typename vector<stl_ptr<T> >::const_iterator at=objs.begin(); at!=objs.end(); ++at) {
                if ((*at)->coord[0] < x_min) x_min = (*at)->coord[0];
                if ((*at)->coord[0] > x_max) x_max = (*at)->coord[0];
                if ((*at)->coord[1] < y_min) y_min = (*at)->coord[1];
                if ((*at)->coord[1] > y_max) y_max = (*at)->coord[1];
                if ((*at)->coord[2] < z_min) z_min = (*at)->coord[2];
                if ((*at)->coord[2] > z_max) z_max = (*at)->coord[2];
        ...
        OCTREE(vector<T*> const& objs,float const& r,float const& xmi,float const& xma,
            ...
            for (typename vector<T*>::const_iterator at=objs.begin(); at!=objs.end(); ++at) {
                if ((*at)->coord[0] < xc_min) continue;
                else if ((*at)->coord[0] > xc_max) continue;
                else if ((*at)->coord[1] < yc_min) continue;
                else if ((*at)->coord[1] > yc_max) continue;
                else if ((*at)->coord[2] < zc_min) continue;
                else if ((*at)->coord[2] > zc_max) continue;
        ...
        bool is_inside(vec3d<float> const& coord) {
            //! Diese Funktion NUR fuer den Wurzelknoten aufrufen!
            if (coord[0] < x_min) return false;
            else if (coord[0] > x_max) return false;
            else if (coord[1] < y_min) return false;
            else if (coord[1] > y_max) return false;
            else if (coord[2] < z_min) return false;
            else if (coord[2] > z_max) return false;
            else return true;
        }


        vector<T*>& get_rel_atoms(float const& max_spacing,vec3d<float> const& coord,
        ...
        OCTREE<T>* p_get_rel_atoms(float const& max_spacing,vec3d<float> const& coord,
                                   unsigned int const& min_objs = 1,unsigned int const& max_objs = 9999999) {
            if ((max_spacing < max_edge && objects.size() > min_objs) || objects.size() > max_objs) {
                if (coord[0] < x_half) { // o1, o3, o5, o7
                    if (coord[1] < y_half) { // o1, o5
                        if (coord[2] < z_half) { // o1
                            ...
                            return o1->p_get_rel_atoms(max_spacing,coord);
                        } else { // o5
                            if (o5 == 0) {
                                o5 = new OCTREE<T>(objects,range,x_min,x_half,y_min,y_half,z_half,z_max,this);
                            }
                            return o5->p_get_rel_atoms(max_spacing,coord);
                        }
                    } else { // o3, o7
                        if (coord[2] < z_half) { // o3
                            if (o3 == 0) {
                            ...
                            return o3->p_get_rel_atoms(max_spacing,coord);
                        } else { // o7
                            if (o7 == 0) {
                                o7 = new OCTREE<T>(objects,range,x_min,x_half,y_half,y_max,z_half,z_max,this);
                            }
                            return o7->p_get_rel_atoms(max_spacing,coord);
                        }
                    }
                } else { // o2, o4, o6, o8
                    if (coord[1] < y_half) { // o2, o6
                        if (coord[2] < z_half) { // o2
                            ...
                            return o2->p_get_rel_atoms(max_spacing,coord);
                        } else { // o6
                            if (o6 == 0) {
                                o6 = new OCTREE<T>(objects,range,x_half,x_max,y_min,y_half,z_half,z_max,this);
                            }
                            return o6->p_get_rel_atoms(max_spacing,coord);
                        }
                    } else { // o4, o8
                        if (coord[2] < z_half) { // o4
                            if (o4 == 0) {
                            ...
                            return o4->p_get_rel_atoms(max_spacing,coord);
                        } else { // o8
                            if (o8 == 0) {
                                o8 = new OCTREE<T>(objects,range,x_half,x_max,y_half,y_max,z_half,z_max,this);
                            }
                            return o8->p_get_rel_atoms(max_spacing,coord);
                        }
                    }
                }
            } else return this;
        ...
        typename vector<T*>::iterator const& begin(float const& max_spacing,vec3d<float> const& coord,
                                                   unsigned int const& min_objs = 1,
                                                   unsigned int const& max_objs = 9999999) {
            if (is_inside(coord)) {
                rel_tree = p_get_rel_atoms(max_spacing,coord,min_objs,max_objs);
