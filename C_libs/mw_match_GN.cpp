...
//                             << " : setting " << loc_max << endl;
                        ...
                        for (vector<MW_EDGE*>::iterator pe=processed_edges.begin(); pe!=processed_edges.end(); ++pe) {
                            if (*pe == curr->adjacent[0] ||
                                *pe == curr->adjacent[1] ||
                                *pe == curr->adjacent[2] ||
                                *pe == curr->adjacent[3]) continue;
...
//                cerr << " -> setting new label " << loc_max << endl;
            ...
            if (pq.size() < 22) {
                tr1::unordered_set<MW_EDGE*> end_set;
...
//                if (end_set.size() > max_end_set) max_end_set = end_set.size(); // DEBUG
                ...
                if (end_set.size() < 22) {
                    ...
                    for (tr1::unordered_set<MW_EDGE*>::iterator et=end_set.begin(); et!=end_set.end(); ++et) {
                    ...
                    } else for (list<int>::iterator l=label_list.begin(); l!=label_list.end(); ++l) {
